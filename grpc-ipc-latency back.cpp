...
int main(int argc, const char *argv[])
{
    ...
    { // child
        ...
        int testDurationMili = chrono::duration_cast<chrono::milliseconds>(end - start).count();
        ...
        // Output results
        std::cout << "Minimum Latency: " << ((*minLatency >= 1000) ? (double) *minLatency/1000 : *minLatency) << ((*minLatency >= 1000) ? " sec" : " us" ) << std::endl;
        std::cout << "Maximum Latency: " << ((*maxLatency >= 1000) ? (double) *maxLatency/1000 : *maxLatency) << ((*maxLatency >= 1000) ? " sec" : " us" ) << std::endl;
        std::cout << "Average Latency: " << ((averageLatency >= 1000) ? (double) averageLatency/1000 : averageLatency) << ((averageLatency >= 1000) ? " sec" : " us" ) << std::endl;
